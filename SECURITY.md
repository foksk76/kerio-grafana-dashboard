# Security Policy

## Scope

This repository is intended primarily for educational, lab, reproducible, and reference implementation use unless explicitly stated otherwise.

It contains Grafana provisioning and dashboard configuration. It does not include Kerio Connect, Elasticsearch, or Logstash binaries.

## Reporting a vulnerability

Please do **not** open a public issue for sensitive security problems.

Instead, report the issue privately to the maintainer and include:

- repository name;
- affected file or component;
- issue description;
- reproduction details;
- potential impact;
- suggested mitigation if available.

## What to include

Please provide:

- affected versions or commit range if known;
- step-by-step reproduction;
- proof of concept if safe to share;
- logs or screenshots if they help explain the issue.

## Handling expectations

Security reports are reviewed on a best-effort basis. No formal SLA is guaranteed.

## Safe sharing rules

Do not publicly share:

- real Grafana admin passwords;
- Elasticsearch credentials;
- private mail content;
- non-anonymized production logs;
- vendor-restricted software artifacts;
- sensitive infrastructure details that are not required to explain the issue.

## Secret handling

- Commit `.env.example`, not `.env`.
- Rotate credentials if a real `.env` is committed by mistake.
- Redact dashboard screenshots if they show private addresses, hostnames, credentials, or sensitive operational data.
